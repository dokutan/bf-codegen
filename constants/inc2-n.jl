...
function generate_simulation_cache()::MMatrix{256, 41, CInt}
    "Generate an MMatrix describing for how many iterations a brainfuck loop `L[al]` runs."
    simulation_cache = zeros(MMatrix{256, 41, CInt})

    for il::CInt in [0:255;]
        for al::CInt in [-20:20;]
            ...
            simulation_cache[il+1, al+21] = iterations
        end
    end
...
function simulate(
    simulation_cache::MMatrix{256, 41, CInt},
    i1::CInt,
    il::CInt,
    a1::CInt,
    al::CInt,
)::CInt
    iterations::CInt = simulation_cache[il+1, al+21]
...
function inc2_n()
    ...
    total_iterations = 41 * 256 * 41 * 41 # i1 * il * a1 * al
    ...
    for i1::CInt in [-20:20;]
        for il::CInt in [0:255;]
            for a1::CInt in [-20:20;]
                for al::CInt in [-20:20;]
                    r::Int = simulate(simulation_cache, i1, il, a1, al)
